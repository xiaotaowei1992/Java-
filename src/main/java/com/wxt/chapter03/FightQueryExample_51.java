...
public class FightQueryExample_51 {
	// 1.合作的各大航空公司
	...
	private static List<String> search(String origin, String dest) {
		...
		// 2. 创建查询航班信息的线程列表
		List<FightQueryTask_51> tasks = fightCompany.stream()
		...
		tasks.stream().map(FightQuery_51::get).forEach(result::addAll);
		return result;
	}

	private static FightQueryTask_51 createSearchTask(String flight,
													  String origin, String dest) {
		return new FightQueryTask_51(flight, origin, dest);
	}
}
